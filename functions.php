...
function getHeader(){
  ...
  $topics = "select distinct cast(start_time as date) date from intraday order by 1";
  $c = setConn();
...
function getScheduleSummary($day){
  $uname = $_SESSION['username'];
  $schedSum = "exec getScheduleSummary '$uname', '$day'";
  $c = setConn();
...
and isnull(n.SignUpNet, 0) < (isnull(n.OrigNet, 0) + isnull(n.Tolerance, 0))
...
do{
    ...
    echo "</td><td>";
    echo "<span>";
    echo '<input type="checkbox" name="cbinterval[]" value="'.$i1[0]->format('Y-m-d').','.$i1[1].'">';
    echo "</span></td>";
    echo '</tr>';
	}
